...
unsigned short _log2(unsigned short size){
...
void fft(struct Image input, struct CImage output){
	...
	unsigned short v, u;             // wave components
	unsigned short y, x;             // spatial component
	float modulator;
...
void _ctft(float *re, float *im, unsigned short size, unsigned short d){
    
    // delarations and inits
    unsigned short base = _log2(size);           // radix-2 division points
    unsigned short l1 = 0;                       // radix-2 indexer
    unsigned short l2 = 0;                       // radix-2 indexer
    ...
    for(unsigned short radix = 0; radix < base; ++radix){
        l1 = l2;
        l2 <<= 1;
        ...
        mod = 0.0;
        for(unsigned short j = 0; j < l1; ++j){           // modulate
            mod_re = cosf(mod);
            mod_im = sinf(mod);
            
            for(unsigned short k = j; k < size; j += l2){   // compute
...
void _ctftrc(struct CImage input, unsigned short direction){
...
void lowPass(struct CImage input, struct Kernel filter){
    for(unsigned short i = 0; i <  input.size; ++i){
        for(unsigned short j = 0; j <  input.size; ++i){
